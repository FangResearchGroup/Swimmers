function [y,H]=gaussfilt2(x,Wn,padding)
% Usage: [y,H]=gaussfilt2(x,Wn,padding)

% adapted from gaussfilt3. Rather than using a taylored sharp filter,
% this gaussfilt2 uses a gaussian kernal to filter the data.
% gaussfilt2 filters array "x",
% returning the results in "y". The filter is gaussian with a half-width
% half-maximum being "Wn" (normalized by the Nyquist frequency = half 
% the sampling frequency). 
% The filter's frequency transfer function is returned in "H".
...
% Output values 
% y: filterd x
% Hd: filter'f frequency transfer function


padding_default = 0;

if nargin<2
    error(['Usage: [y,H] = ' mfilename '(x,Wn,padding'])
end
...
sigma = Wn/sqrt(2*log(2));

arg = (W1.*W1)/(sigma*sigma) + (W2.*W2)/(sigma*sigma);
hh = exp( -arg/2 );
...
h = fftshift(ifftn(hd)); % this is the gaussian kernal
